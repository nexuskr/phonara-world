...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!roomId || !isChatOpen || !user) return;
    ...
    <div className="min-h-screen pb-24">
      ...
      {user && (
        <>
          ...
          <button
            onClick={toggleChat}
            className="fixed bottom-28 right-6 z-50 w-14 h-14 rounded-2xl glass-strong neon-border flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 group"
          >
            <MessageCircle className="w-7 h-7 text-cyber-blue group-hover:text-neon-orange transition-colors" />
            {isEmpireRoom && (
              <div className="absolute -top-1 -right-1 w-5 h-5 bg-gold rounded-full flex items-center justify-center text-[10px] font-black text-black animate-pulse shadow-inner">
                👑
              </div>
            )}
          </button>
          ...
          {/* Chat Panel */}
          {isChatOpen && (
            <div className="fixed bottom-48 right-6 z-[999] w-[380px] h-[520px] glass-strong neon-border bg-black/95 backdrop-blur-3xl rounded-3xl flex flex-col shadow-2xl overflow-hidden border border-white/10">
              {/* Header */}
              <div
              ...
              >
                ...
                <h2
                  className={`font-bold tracking-tighter flex-1 ${
                    isEmpireRoom ? "text-gold neon-glow-purple" : "text-cyber-blue neon-glow"
                  }`}
                >
              ...
              <form onSubmit={sendMessage} className="p-4 border-t border-white/10 bg-black/70">
                <div className="flex gap-2">
                  <Input
                    ...
                    className="glass-strong flex-1 text-white placeholder:text-white/40"
                  />
                  <Button
                    type="submit"
                    className={`px-8 font-bold ${isEmpireRoom ? "bg-gold hover:bg-yellow-400 text-black" : "bg-neon-orange hover:bg-orange-500"}`}
                  >
                    SEND
                  </Button>
                </div>
